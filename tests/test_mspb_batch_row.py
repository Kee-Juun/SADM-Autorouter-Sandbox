"""No-driver contract tests for the Phase 6D MSPB row wrapper."""
...
from core.router_modes.mspb_batch_row import (
    MspbRowOutcome,
    process_mspb_document_row,
)
from core.smducar_router import CaseLawRouter


class MspbBatchRowTests(unittest.TestCase):
    ...
    def test_success_preserves_attempt_search_extract_record_and_click(self):
        ...
        outcome = process_mspb_document_row(
            router,
            handler,
            17,
            row,
    ...
    def test_dependency_errors_propagate_to_outer_batch_boundary(self):
        ...
        handler.extract_metadata.side_effect = error
    ...
    def test_router_method_is_a_thin_compatibility_wrapper(self):
        ...
        with patch(
            "core.smducar_router.run_process_mspb_document_row",
            return_value=expected,
        ) as delegated:
            actual = CaseLawRouter.process_mspb_document_row(
                router,
                handler,
                27,
                row,
