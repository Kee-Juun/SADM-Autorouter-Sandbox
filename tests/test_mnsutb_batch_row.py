"""No-driver contract tests for the Phase 6E MNSUTB row wrapper."""
...
from core.router_modes.mnsutb_batch_row import (
    MnsutbRowOutcome,
    process_mnsutb_document_row,
)
from core.smducar_router import CaseLawRouter


class MnsutbBatchRowTests(unittest.TestCase):
    ...
    def test_success_preserves_attempt_search_extract_record_and_click(self):
        ...
        outcome = process_mnsutb_document_row(
            router,
            handler,
            17,
            row,
    ...
    def test_router_method_is_a_thin_compatibility_wrapper(self):
        ...
        with patch(
            "core.smducar_router.run_process_mnsutb_document_row",
            return_value=expected,
        ) as delegated:
            actual = CaseLawRouter.process_mnsutb_document_row(
                router,
                handler,
                27,
                row,
