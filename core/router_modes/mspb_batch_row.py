"""MSPB document-row orchestration behind the legacy router boundary."""
...
class MspbRowOutcome:
    """Data returned to the outer batch loop after MSPB preparation."""
...
def process_mspb_document_row(
    ...
    """Prepare one MSPB row while leaving shared batch state to the caller."""
    ...
    if not metadata:
        missing_policy = get_missing_metadata_policy("mspb")
        row_handler.record_metadata(
            router,
            full_index,
            row,
        ...
        logging.warning(
            "Skipping MSPB row %d: required PDF metadata could not be "
            "extracted.",
            full_index + 2,
        )
        return MspbRowOutcome(
