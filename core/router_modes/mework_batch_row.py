"""MEWORK document-row orchestration behind the legacy router boundary."""
...
class MeworkRowOutcome:
    """Data returned to the outer batch loop after MEWORK preparation."""
...
def process_mework_document_row(
    ...
    """Prepare one MEWORK row while leaving shared batch state to the caller."""
    ...
    if not metadata:
        missing_policy = get_missing_metadata_policy("mework")
        row_handler.record_metadata(
            router,
            full_index,
            row,
        ...
        logging.warning(
            "Skipping MEWORK row %d: required PDF metadata could not be "
            "extracted.",
            full_index + 2,
        )
        return MeworkRowOutcome(
    ...
    return MeworkRowOutcome(continue_to_form=True, metadata=metadata)
